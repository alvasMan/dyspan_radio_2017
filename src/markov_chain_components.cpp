...
DeepLearningMarkovChain::DeepLearningMarkovChain(size_t num_states, float diag_prob) : 
    transition_probability(num_states,num_states), state_probability(num_states,1.0/num_states), tmp_prob(num_states)
{
    for(auto &e : transition_probability)
        e = (1-diag_prob)/ (num_states-1);

    for(size_t i = 0; i < num_states; ++i)
        transition_probability.at(i,i) = diag_prob;
}
    
void DeepLearningMarkovChain::push(const vector<float>& deep_learning_beliefs)
{
    for(int j = 0; j < n_states(); ++j)
    {
        tmp_prob[j] = -std::numeric_limits<float>::max();
        for(int i = 0; i < n_states(); ++i)
        {
            float prb = state_probability[i] * deep_learning_beliefs[j] * transition_probability.at(i,j);
            if(prb > tmp_prob[j])
            {
                tmp_prob[j] = prb;
            }
    ...
    // normalize
    float max_val = *max_element(tmp_prob.begin(), tmp_prob.end());
    transform(tmp_prob.begin(), tmp_prob.end(), state_probability.begin(), [&max_val](float f)
    {
        return f/max_val;
    });
}

void DeepLearningModeCounter::push(int8_t scenario)
{
    if(scenario_guesses.size()==scenario_guesses.capacity())
        counter_scenarios[scenario_guesses.front()]--;
    scenario_guesses.push_back(scenario);
    counter_scenarios[scenario]++;
}
