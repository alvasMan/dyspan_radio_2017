...
ModulationSearchApi::changeOfdmMod()
{
    ...
    if(su_provided == 0)
        return std::make_tuple(false,*m_current_modulation); //No SU trhoughput offered, can't do anything with this.
    ...
    else //The gain has not changed, carry on searching
    {
        if(m_sample_counter < m_mod_samples)
        {
            ...
            cout << "\t"<<su_throughput<<" "<<m_sample_counter<<endl;
    ...
    //std::cout << "\t" << "Previous SU Provided Throughput: " << m_previous_tsu << std::endl;
	...
	return std::make_tuple(m_stop_searching, *m_current_modulation);
}
